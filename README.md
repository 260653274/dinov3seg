...
```
            输入图像 (B, 3, H, W)，H,W 为 16 的倍数
                            │
              ┌─────────────▼─────────────┐
              │  DINOv3 ViT-S/16 (冻结)   │   21 M 参数
              │  块标记，嵌入维度=384     │
              └──────┬─────────────┬──────┘
        块 6 │             │ 最后一块
   (B,384,h,w) │             ▼ (B,384,h,w)，h=H/16, w=W/16
                │      ┌──────────────┐
                │      │  金字塔池化  │  1×1, 2×2, 3×3, 6×6
                │      │  模块 (PPM)  │  每个分支缩减至 96 通道
                │      │  + 上采样    │  与输入拼接 → (B,768,h,w)
                │      └──────┬───────┘
                │             ▼
                │      ┌──────────────┐
                │      │ Conv 3×3 256 │
                │      │ BN, ReLU     │       2.81 M 可训练参数
                │      │ Dropout 0.1  │
                │      │ Conv 1×1 21  │
                │      └──────┬───────┘
                │             │ logits (B,21,h,w)
                │             ▼
                │   双线性插值 ↑16  →  (B,21,H,W) ── 主损失 (CE, ignore=255)
                │
                ▼
          ┌──────────────┐
          │ 辅助头部     │  Conv3×3 → BN → ReLU → Drop → Conv1×1
          │ (仅训练)     │  上采样至 (B,21,H,W)  ── 辅损失 × 0.4
          └──────────────┘
```
